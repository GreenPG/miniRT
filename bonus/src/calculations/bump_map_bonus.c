...
/*   Updated: 2023/06/16 15:06:05 by gtouzali         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_normal	bump_sphere(t_sphere *sphere, t_vector vec, t_normal normal, mlx_texture_t	*tex)
{
    ...
    derivative_u = tex->pixels[pixel_index] / 255;
	derivative_v = tex->pixels[pixel_index] / 255;
	pixel_index = (((y + 1) % tex->height)  * tex->width + x) * tex->bytes_per_pixel;
	derivative_u = tex->pixels[pixel_index] / 255 - derivative_u; 
	pixel_index = (y * tex->width + (x + 1) % tex->width) * tex->bytes_per_pixel;
	derivative_v = tex->pixels[pixel_index] / 255 - derivative_v;
