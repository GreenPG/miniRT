...
/*   Updated: 2023/05/08 17:14:22 by gpasquet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	cylinder_hit(t_cylinder *cylinder, t_vector ray)
{
	...
	double		c;
	t_vector	upper_caps;
	t_vector	ray_to_axis;	
	
	upper_caps = scalar_multiplication(cylinder->vector, cylinder->height / 2);
	ray_to_axis = vector_cross(vector_cross(*cylinder->vector, upper_caps), *cylinder->vector);
	a = dot_product(ray, ray);
	b = 2 * dot_product(ray_to_axis, ray);
	c = dot_product(ray_to_axis, ray_to_axis) - pow(cylinder->diameter / 2, 2);
